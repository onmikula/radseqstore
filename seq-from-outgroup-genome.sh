...
#PBS -N seq-from-outgroup-genome
...
bedtools getfasta -s -fi "$genomefile" -bed "$bedfile" -fo "$outgroupseq"

# attach retrieved sequences
Rscript --vanilla "$prepare_retrieved_sequences" "$outgroupseq" "$seqfile" "$format" "$bedfile"

# get genomic coordinates of ddrad loci
Rscript --vanilla "$get_genomic_coordinates" "$filteredbed"

# align outgroup sequences by MAFFT
while read loc
do
...
mv "$outgroupseq" "output/$outgroupseq"
mv "$nexfile" "output/$nexfile"
mv "$filterednex" "output/$filterednex"
